...
// Pin definitions
#define ANEMOMETER_PIN 34      // GPIO34 for anemometer (analog) ✅ WORKING
#define GEIGER_PIN 13          // GPIO13 for Geiger counter ⚡ CORRECTED
...
void setup() {
  ...
  Serial.println("============================================");
  Serial.println("🌪  Anemometer: GPIO34 (Analog) ✅");
  Serial.println("☢️  Geiger Counter: GPIO13 ⚡ EXPERT CODE!");
  Serial.println("============================================");
  ...
  Serial.println("✅ GPIO13 configured (EXPERT METHOD)");
  Serial.println("✅ GPIO34 configured for Anemometer");

  delay(1000);
  int initialState = digitalRead(GEIGER_PIN);
  Serial.printf("📍 GPIO13 initial state: %s\n", initialState ? "HIGH" : "LOW");
...
void loop() {
  ...
  if ((currentMillis - cpmTime) >= 60000) {
    ...
    Serial.printf("⏱  Time: %.1f minutes\n", currentMillis / 60000.0);
    Serial.printf("⚡ Radiation Pulses: %lu\n", CPM);
    Serial.printf("📊 CPM: %lu\n", CPM);
    Serial.printf("☢️  Radiation: %.3f µSv/h\n", radiationLevel);
    Serial.printf("🌪  Wind: %.2f m/s (%.2f km/h)\n", windSpeed_mps, windSpeed_kmh);
    Serial.println("=======================================\n");
  }
